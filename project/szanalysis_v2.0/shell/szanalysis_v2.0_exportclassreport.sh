#!/bin/sh
# ===================================================================================  
# Filename:    szanalysis_v2.0_unitclassofreport.sh
# Revision:    2.0
# Date:        2015/0/06
# Author:      zhengsh
# Description: Analysis of demand in Shenzhen
# Notes:       szanalysis_v2.0_unitclassofreport.sh 00 day
# ===================================================================================
...
function main()
{
    ...
    OP_TABLE_NAME=tmp_day_class
    FILENAME="UserVisitsClasses_${OP_EDAY}.txt"
    rm -rf ${OUTPUTPATH}/${FILENAME}

    fecho "Gets the ${OP_EDAY} execution log of szanalysis_v2.0_unitclassofreport.sh"
...
select * from (select zz.userid,zz.parent_name,yy.class_name,zz.tt1,yy.tt2,zz.rnop,row_number() over(partition by yy.userid, yy.parent_name order by yy.tt2 desc) as rno
          from (select userid, parent_name, tt1,rnop from (select userid,parent_name,row_number() over(partition by bb.userid order by tt1 desc) as rnop,tt1
                          from (select t.userid, parent_name, sum(frequency) tt1 from tw_static_useracc t
                 ...
                 where rnop < 4) zz
          left join (select t.userid,parent_name,class_name,sum(frequency) tt2 from tw_static_useracc t join (select class_id, parent_name, class_name from aotain_dim.to_url_class where parent_id is not null group by class_id, parent_name, class_name) g
                        on t.attvalue = g.class_id
