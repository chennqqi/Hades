#!/bin/sh
# ===================================================================================  
# Filename:    szanalysis_v2.0_unitofreport.sh
# Revision:    2.0
# Date:        2015/0/06
# Author:      zhengsh
# Description: Analysis of demand in Shenzhen
# Notes:       szanalysis_v2.0_unitofreport.sh 00 day
# ===================================================================================


AOTAIN_SZUSERANALYSIS_LOG=/root/work/log/szuseranalysis.log
...
function main()
{
    ...
    if [ $# -ne 2 ]; then
        ...
        echo "eg, ./gethourreport.sh 00 day"
        return 1;
    fi

    # define parameters    
    OP_EDAY=`date +"%Y%m%d" --date="-1 day"`
    OP_TABLE_NAME=tmp_hour_visits_$1
    if [ "$2" = "month" ]; then
...
select t.userid,g.class_name,t.frequency,row_number() over(partition by userid,substring(lasthour,9,2) order by frequency desc) as rno 
from aotain_dim.to_ref_domainclass g  join aotain_dw.tw_static_useracc t on attvalue = class_id
where partdate >= '${OP_SDAY}' and partdate <= '${OP_EDAY}' and attcode='URLNo' and substring(lasthour,9,2) = '$1' and attvalue != 0 ) ta where rno <4 group by userid ) r1;"
