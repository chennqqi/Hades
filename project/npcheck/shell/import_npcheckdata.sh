...
from sharev5_check_log a where log_time>=to_date('$STARTDATE','yyyy-mm-dd') and log_time<=to_date('$DATE','yyyy-mm-dd')
group by user_name
union all
...
#mergefile
cat $OUTPUTPATH/part* > $OUTPUTPATH/1_N"$DATE"0000_WEEK.csv
gzip $OUTPUTPATH/1_N"$DATE"0000_WEEK.csv
