...
public class Phone7FileDriver extends Configured implements Tool{
	...
	public int run(String[] args) throws Exception {
        ...
        job.setJobName("Phone7 File[" + date + "]");                    
        job.setJarByClass(getClass());

        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(Text.class);
        job.setReducerClass(Phone7FileReducer.class);
	    ...
	    for(int i=0; i<inputPaths.length; i++)
        {
            ...
            if(p[1].equals("orc"))
        	{
        		MultipleInputs.addInputPath(job,new Path(p[0]),OrcNewInputFormat.class,Phone7GetFileMapper.class);
            	System.out.println("-------------orcpath: "+p[0]);
        	}
        	else
        	{
        		MultipleInputs.addInputPath(job,new Path(p[0]),TextInputFormat.class,Phone7PostFileMapper.class);
            	System.out.println("-------------txtpath: "+p[0]);
        	}
        }
	...
	public static void main(String[] args)throws Exception{
        int exitcode = ToolRunner.run(new Phone7FileDriver(), args);
