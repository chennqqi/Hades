...
public class PhoneFileDriver extends Configured implements Tool{
	...
	public int run(String[] args) throws Exception {
        ...
        job.setJobName("Rule Phone File[" + date + "]");                    
        job.setJarByClass(getClass());

        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(Text.class);
        job.setReducerClass(PhoneFileReducer.class);
	    ...
	    for(int i=0; i<inputPaths.length; i++)
        {
            ...
            if(p[1].equals("orc"))
        	{
        		MultipleInputs.addInputPath(job,new Path(p[0]),OrcNewInputFormat.class,PhoneGetFileMapper.class);
            	System.out.println("-------------orcpath: "+p[0]);
        	}
        	else
        	{
        		MultipleInputs.addInputPath(job,new Path(p[0]),TextInputFormat.class,PhonePostFileMapper.class);
	...
	public static void main(String[] args)throws Exception{
        int exitcode = ToolRunner.run(new PhoneFileDriver(), args);
