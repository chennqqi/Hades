...
public class PCAnalysisResultDriver extends Configured implements Tool {
    public int run(String[] args) throws Exception {                  
        ...
        job.setMapperClass(PCAnalysisResultMapper.class);
        job.setMapOutputKeyClass(Text.class);
        job.setReducerClass(PCAnalysisResultReducer.class);
        //job.setNumReduceTasks(10);
        job.setMapOutputValueClass(Text.class);                  
        job.setInputFormatClass(TextInputFormat.class);
        //job.setInputFormatClass(LzoTextInputFormat.class);
        for(int i=0;i<inputPaths.length;i++){
        	FileSystem fsTarget1 = FileSystem.get(URI.create(inputPaths[i]),conf);
            Path pathTarget1 = new Path(inputPaths[i]);
            if(fsTarget1.exists(pathTarget1))
            {   
    ...
    public static void main(String[] args)throws Exception{
         int exitcode = ToolRunner.run(new PCAnalysisResultDriver(), args);
