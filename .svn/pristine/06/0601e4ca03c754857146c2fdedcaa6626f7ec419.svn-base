...
#Created by wayne 2015-05-18
#create a history table for app k-v data
...
TABLENAME=tw_user_appinfo
MRCLASS=com.aotain.dw.UserAttDriver
INPUTPATH=/user/hive/warehouse/broadband.db/to_opr_app/$CITY/$DATE
...
hadoop fs -mkdir /user/hive/warehouse/broadband.db/to_opr_app/$CITY 1>>$LOGFILE 2>>$LOGFILE
hadoop fs -mkdir /user/hive/warehouse/broadband.db/to_opr_app/$CITY/$DATE 1>>$LOGFILE 2>>$LOGFILE
...
#mapreduce
hadoop jar $MRPATH/dw.jar $MRCLASS $INPUTPATH"|txt#"$BAKPATH"|txt" $OUTPUTPATH $CONFIG $DATE 1>>$LOGFILE 2>>$LOGFILE
ret=$?
if [ $ret -ne 0 ]; then
