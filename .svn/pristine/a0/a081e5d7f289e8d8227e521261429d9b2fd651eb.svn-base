...
public class TerminalDC {
	...
	public static int TerminalSparkFunc(final String[] args){
		...
		try
		{
			...
			SparkConf sparkConf = new SparkConf().setAppName("Terminal DC Spark");
			sparkConf.set("spark.shuffle.consolidateFiles", "true");
		    Configuration config = new Configuration();
		    config.addResource("/etc/hadoop/conf");
		    config.addResource(new Path("/etc/hadoop/conf"));
					...
					// CommonFunction.findByRegex(kv.get("CALL_TYPE"), "[0-9]*", 0)
					 ...
					 String deviceName = "";
						...
						for(String mapKey : mapConfig.keySet())
						{
							...
							float v = CommonFunction.Similar(items[10],arrDev[1]);
							if(v > fMaxRate && v >= 0.5)
							{
								fMaxRate = v;
						...
						//UserName|OperSys|OperSysVer|Browser|BrowserVer|Device
						//String[] items = v1.split("\\|",-1);
						
						if(items.length == 6 && 
								items[1].equals("Windows") && 
								!items[2].equals("CE"))
						{
							DeviceType = "PC";
						}
				     ...
				     String value = strKey + deviceName + "|" + fullName + "|" + DeviceType + "|";
