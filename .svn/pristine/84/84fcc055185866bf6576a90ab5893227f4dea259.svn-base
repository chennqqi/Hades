#!/bin/sh
# ===================================================================================  
# Filename:    szanalysis_v2.0_unitofctnetreport.sh
# Revision:    2.0
# Date:        2015/0/06
# Author:      zhengsh
# Description: Analysis of demand in Shenzhen
# Notes:       szanalysis_v2.0_unitofctnetreport.sh 00 day
# ===================================================================================
...
function main()
{
    ...
    if [ $# -ne 2 ]; then
        ...
        echo "eg, ./szanalysis_v2.0_unitofctnetreport.sh 00 day"
        return 1;
    fi

    # define parameters    
    OP_EDAY=`date +"%Y%m%d" --date="-1 day"`
    OP_TABLE_NAME=tmp_hour_ctnetvisits_$1
    if [ "$2" = "month" ]; then
    ...
    /usr/bin/hive -S -e "use aotain_dw; create table ${OP_TABLE_NAME} as  select userid,regexp_replace(regexp_replace(CONCAT_WS(',',ds,case when ds[0] is null then \"0\" end,case when ds[1] is null then \"0\" end,case when ds[2] is null then \"0\" end,\"0,0,0\"),'1_|2_|3_',''),',','|') urllist from( select userid,sort_array(collect_set(CONCAT_WS('_',CONCAT_WS('_',cast(rno as String),class_name),cast(frequency as String)))) ds from( select t.userid,g.class_name,t.frequency,row_number() over(partition by userid,substring(lasthour,9,2) order by frequency desc) as rno from aotain_dim.to_ref_domainclass g  join tw_static_areauseracc t on attvalue = class_id where partdate >= '${OP_SDAY}' and partdate <= '${OP_EDAY}' and attcode='URLNo' and substring(lasthour,9,2) = '$1' ) ta where rno <4 group by userid  ) r1;"
