...
public class IdentifierDriver extends Configured implements Tool{
	...
	public int run(String[] args) throws Exception {
	    ...
	    for(int i=0; i< inputPaths.length; i++)
        {
            ...
            if(p[1].equals("orc"))
        	{
        		MultipleInputs.addInputPath(job,new Path(p[0]),OrcNewInputFormat.class,IdentifierORCMapper.class);
            	System.out.println("-------------orcpath: "+p[0]);
        	}
        	else
        	{
        		MultipleInputs.addInputPath(job,new Path(p[0]),TextInputFormat.class,IdentifierTEXTMapper.class);
