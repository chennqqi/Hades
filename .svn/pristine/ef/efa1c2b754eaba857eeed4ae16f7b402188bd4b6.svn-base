#!/bin/sh
# ===================================================================================  
# Filename:    szanalysis_v2.0_ctwapusers.sh  
# Revision:    2.0
# Date:        2015/0/06
# Author:      zhengsh
# Description: Analysis of demand in Shenzhen
# Notes:       szanalysis_v2.0_ctwapusers.sh
# ===================================================================================
...
function main()
{
    ...
    if [ $# -ne 1 ]; then
        ...
        echo "eg, ./szanalysis_v2.0_ctwapusers.sh day"
        return 1;
    fi

    # define parameters
    OP_TABLE_NAME=tmp_day_ctwapusers
    ...
    if [ "$1" = "month" ]; then
        ...
        OP_SDAY=`date +"%Y%m%d" --date="-11 day"`
    else
    ...
    /usr/bin/hive -S -e "use aotain_dw; create table ${OP_TABLE_NAME} as  select distinct userid from tw_static_wapuseracc where partdate >= '${OP_SDAY}' and partdate <= '${OP_EDAY}';"
