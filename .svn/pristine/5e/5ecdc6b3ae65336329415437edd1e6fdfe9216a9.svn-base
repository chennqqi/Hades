...
where a.start_time>=to_date('$DATE','yyyy-mm-dd')-6 and a.end_time<=to_date('$DATE','yyyy-mm-dd')
group by a.user_name) where  \$CONDITIONS " -m 1 --target-dir $HFDSPATH

hadoop fs -get  $HFDSPATH/part*  ${OUTPUTPATH}/
#mergefile
cat $OUTPUTPATH/part* > $OUTPUTPATH/DIST"$DATE"0000_WEEK.csv
gzip $OUTPUTPATH/DIST"$DATE"0000_WEEK.csv
