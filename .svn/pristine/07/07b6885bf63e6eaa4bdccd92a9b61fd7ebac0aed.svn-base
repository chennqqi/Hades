...
LOGFILE=${WORKPATH}/log/HBase_HTTP_PUSH_FEEDBACK_${CITYNAME}_${DATE}.log
CONFIGPATH=${WORKPATH}/config/

echo `date +"%Y-%m-%d %H:%M:%S"`  "------PUSH_FEEDBACK------------" | tee -a $LOGFILE
...
#HDFS--
DATAPATH=/user/hive/warehouse/broadband.db/ipsfeedback/${CITYNAME}/${DATE}
...
#hadoop jar $JAVALIB/feedbackV2.jar com.aotain.push.FeedbackIntoHbase ${DATAPATH} ${CITYNAME}_${DATE} 
hadoop jar $JAVALIB/feedbackV2.jar com.aotain.push.FeedbackIntoHbase ${DATAPATH} ${CITYNAME}_${DATE} 1>>$LOGFILE 2>>$LOGFILE
